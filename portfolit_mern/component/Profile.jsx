...
import { RotatingTriangles, ProgressBar, ThreeDots, TailSpin } from 'react-loader-spinner';
        ...
        <>
          <div className='uper_profile'>
            <div className='user_profile_image_div'>
              {
                user ? (
                  <img src={user.photo} alt="User Photo" onError={(e) => { e.target.src = 'default.jpg'; }} />
                ) : 
                <img src='default.jpg' alt="Default User Photo" /> // Fallback image if user is not defined
              }
              {
                user ? (
                  <p className='user_profile_name'>{user.username}</p>
                ) : (
                  <ProgressBar
                    visible={true}
                    height="80"
            ...
            <div className='profile_text'>
              {
                user ? (
                  ...
                  <h1 className='profile_name'>Loading...</h1>
                )
              }
              {
                user ? <p>{user.bio}</p> : <p>.......</p> 
              }
              {
                <p>projects =  {project?.length ? project?.length : "*"}</p>
                // project ? (<p>projects = {project?.length}</p>) : <p>......</p>
              }
