...
function Project() {
  ...
  const delete_p = async (image, id) => {
    ...
    if (y_n == true) {
      const url = `${backend_url}/deletefile/${id}/image/${encodeURIComponent(image)}`;
        ...
        <div>
...

          {
            project.length > 0 ? (
                  ...
                  <div data-aos="zoom-in" className='project_div' >
                    ...
                    <a href={element.online_link} target='_blanck'>
                      <div className='project_div_image'>
                        <img src={element.image} alt="Porject demo" onError={(e) => { e.target.src = 'default_project.jpg'; }} />
                      </div>
                    </a>
                  </div>
                </>
              ...
              :
              (
                project?.length == 0 ? (<p>No project available</p>) :
                  <>
                    <MutatingDots
                      visible={true}
                      height="100"
